...
from src.config import GENERATOR_SETTINGS, RERANKER_ENABLED
from src.evaluator import load_dataset
from src.generator import (
    FIXED_N_CTX, MAX_NEW_TOKENS, TEMPERATURE, TOP_P,
...
RESULT_CSV = ROOT_DIR / "results" / "step7c_300_results.csv"
SUMMARY_JSON = ROOT_DIR / "results" / "step7c_summary.json"
REPORT_MD = ROOT_DIR / "results" / "step7c_report.md"
MANUAL_CSV = ROOT_DIR / "results" / "step7c_manual_review_sample.csv"
SMOKE_JSON = ROOT_DIR / "results" / "step7c_smoke_summary.json"
SMOKE_CSV = ROOT_DIR / "results" / "step7c_smoke_results.csv"
RESUME_TEST_CSV = ROOT_DIR / "results" / "step7c_resume_test.csv"
SMOKE_IDS = {
    "english": ("Q002", "Q005", "Q007", "Q009", "Q010"),
    "bangla": ("Q002", "Q009", "Q013", "Q026", "Q028"),
    "banglish": ("Q002", "Q005", "Q007", "Q009", "Q013"),
}
...
def evaluate(rows: list[dict[str, Any]], output_path: Path, initial_rows: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    ...
    for position, row in enumerate(rows, start=1):
        ...
        prompt_tokens = max((int(item.get("prompt_tokens", 0)) for item in telemetry), default=0)
        ...
        item = {
            ...
            "tokens_per_second": completion_tokens / float(result.get("latency_seconds", {}).get("generation", 0.0)) if completion_tokens and float(result.get("latency_seconds", {}).get("generation", 0.0)) else 0.0,
        ...
        eta = run_elapsed / position * (len(rows) - position)
        print(f"answered {total_position}/{total_expected} {row.get('expected_language')} {row.get('question_id')} elapsed={run_elapsed:.1f}s eta={eta:.1f}s", flush=True)
        if total_position % 5 == 0 or position == len(rows):
...
def read_checkpoint(path: Path) -> list[dict[str, Any]]:
    ...
    integer_fields = {"generation_attempts", "prompt_tokens", "completion_tokens", "rss_bytes", "available_ram_bytes", "swap_used_bytes"}
    float_fields = {
        "automatic_precision", "automatic_recall", "automatic_f1", "retrieval_seconds", "answering_seconds",
        "generation_seconds", "retry_seconds", "total_seconds", "tokens_per_second",
    }
    for row in rows:
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    prompt_tokens = [int(row["prompt_tokens"]) for row in generated if int(row.get("prompt_tokens", 0))]
    token_rates = [float(row["tokens_per_second"]) for row in generated if float(row.get("tokens_per_second", 0.0))]
    return {
        ...
        "tokens_per_second_median": statistics.median(token_rates) if token_rates else 0.0,
        "memory": {
            "peak_rss_bytes": max((int(row.get("rss_bytes", 0)) for row in rows), default=0),
            "minimum_available_ram_bytes": min((int(row.get("available_ram_bytes", 0)) for row in rows), default=0),
            "peak_swap_used_bytes": max((int(row.get("swap_used_bytes", 0)) for row in rows), default=0),
        },
        "errors": sum(bool(row.get("error")) for row in rows),
        "qwen": {"model": "Qwen2.5-7B-Instruct", "quantization": "Q4_K_M", "profile": GENERATOR_SETTINGS.profile, "context": FIXED_N_CTX, "temperature": TEMPERATURE, "top_p": TOP_P, "max_tokens": MAX_NEW_TOKENS},
    }
...
def manual_sample(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for language in ("english", "bangla", "banglish"):
        ...
        candidates.sort(key=lambda row: (not bool(row["generation_used"]), str(row["question_id"])))
        chosen = candidates[:10]
        fields = (
            "language", "question", "reference", "final_answer", "answer_strategy",
            "generation_used", "language_validation_passed", "grounding_validation_passed",
            "source", "page", "supporting_excerpt",
        )
        human_fields = (
            "correctness", "relevance", "groundedness", "completeness", "naturalness",
            "semantic_consistency", "error_category", "notes",
        )
        sample.extend({**{key: row[key] for key in fields}, **{key: "" for key in human_fields}} for row in chosen)
    return sample
...
def write_report(summary: dict[str, Any]) -> None:
    ...
    lines.extend([
        ...
        "## Prompt tokens", "", json.dumps(summary["prompt_tokens"], ensure_ascii=False), "",
        "## Memory", "", json.dumps(summary["memory"], ensure_ascii=False), "",
    ])
    REPORT_MD.write_text("\n".join(lines), encoding="utf-8")


def select_smoke(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    lookup = {(str(row["question_id"]), str(row["expected_language"])): row for row in rows}
    return [lookup[(question_id, language)] for language, ids in SMOKE_IDS.items() for question_id in ids]


def rows_to_resume(dataset: list[dict[str, Any]], checkpoint: list[dict[str, Any]]) -> list[dict[str, Any]]:
    keys = {(str(row.get("question_id")), str(row.get("language"))) for row in checkpoint}
    return [row for row in dataset if (str(row.get("question_id")), str(row.get("expected_language"))) not in keys]
...
if __name__ == "__main__":
    ...
    if args.resume_test:
        if RESUME_TEST_CSV.exists():
            RESUME_TEST_CSV.unlink()
        first = dataset[:5]
        evaluate(first, RESUME_TEST_CSV)
        checkpoint = read_checkpoint(RESUME_TEST_CSV)
        target = dataset[:10]
        rows = evaluate(rows_to_resume(target, checkpoint), RESUME_TEST_CSV, initial_rows=checkpoint)
        keys = {(row["question_id"], row["language"]) for row in rows}
        if len(rows) != 10 or len(keys) != 10:
            raise RuntimeError("Resume validation found duplicate or missing rows.")
        print("resume-test passed: 10 unique rows; no duplicates or missing rows")
    elif args.smoke_only:
        selected = select_smoke(dataset)
        checkpoint = read_checkpoint(SMOKE_CSV) if args.resume else []
        rows = evaluate(rows_to_resume(selected, checkpoint), SMOKE_CSV, initial_rows=checkpoint)
        summary = summarize(rows)
        SMOKE_JSON.write_text(json.dumps({"summary": summary, "rows": rows}, ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        rows = evaluate(rows_to_resume(selected, checkpoint), RESULT_CSV, initial_rows=checkpoint)
        summary = summarize(rows)
